...
/***/function (module, exports, __webpack_require__) {
	...
	var owlCarousel = __webpack_require__(6);
	...
	var Carousels = function () {
		...
		_createClass(Carousels, [{
			key: 'initDesktopCarouselOtherClothes',
			value: function initDesktopCarouselOtherClothes() {
			...
			key: 'initMobileCarouselBaseImages',
			value: function initMobileCarouselBaseImages() {
...
/***/function (module, exports, __webpack_require__) {
	...
	var PreviewImageForComments = function () {
		function PreviewImageForComments() {
			_classCallCheck(this, PreviewImageForComments);
		...
		_createClass(PreviewImageForComments, [{
			key: 'setImage',
			value: function setImage() {
				...
				this.input.addEventListener("change", function () {
					...
					if (_this.input.files[0].type !== 'image/png' && _this.input.files[0].type !== 'image/jpeg') {
						_this.input.value = '';
						return;
					}
					...
					fileReader.addEventListener("load", function (event) {
						...
						_this.imageContainer.style.backgroundImage = 'url(' + src + ')';
						_this.imageContainer.style.backgroundSize = 'cover';
						_this.imageContainer.style.backgroundPosition = 'center center';
					}, false);

					fileReader.readAsDataURL(_this.input.files[0]);

					_this.photoText.innerHTML = 'Удалить фото';
				}, false);
				...
				this.input.addEventListener('click', function (event) {
					...
					_this.photoText.innerHTML = 'Загрузить фото';

					_this.imageContainer.style.backgroundImage = '';
				});
		...
		return PreviewImageForComments;
	}();
	...
	var pageDraw = __webpack_require__(2);

	var loadComments = function () {
		function loadComments() {
			_classCallCheck(this, loadComments);

			this.title = document.title;
			this.page = 1;

			this.firstScriptStart = true;
			...
			if (window.location.search) {
				...
				if (queryParsed['comments_page']) {
					this.page = +queryParsed['comments_page'];
				}
			}

			this.path = window.location.pathname + '?link=comments&comments_page=';
			this.histotyPath = window.location.pathname + '?comments_page=';

			this.comments = new pageDraw({
				position: ["afterend", document.querySelector('.productCard__comments-head')]
			});
		...
		_createClass(loadComments, [{
			...
			key: 'chengeCommentsPage',
			value: function chengeCommentsPage(event) {
				var _this2 = this;

				this.page = event && +event.currentTarget.innerHTML > 0 && +event.currentTarget.innerHTML || this.page;

				if (!this.firstScriptStart) {
					if (!window.location.search) {
				...
				this.comments.data.then(function (data) {
					...
					_this2.pages.content = _this2.pagesContent(data);

					var buttons = _this2.pages.content.querySelectorAll('[data-commentsPageNumber]');
					if (!buttons) {
						return;
					}
					for (var i = 0; i < buttons.length; i++) {
						buttons[i].addEventListener('click', function (event) {
							_this2.chengeCommentsPage(event);
						});
					}
				});
	...
	var commentConstructorButtons = function () {
		function commentConstructorButtons() {
			var _this3 = this;

			_classCallCheck(this, commentConstructorButtons);
			...
			this.setButton.addEventListener('click', function () {
				return _this3.setButtonLogic();
			});

			this.closeButton.addEventListener('click', function () {
				return _this3.closeButtonLogic();
			});
		...
		_createClass(commentConstructorButtons, [{
			key: 'setButtonLogic',
			value: function setButtonLogic() {
				var _this4 = this;
				...
				var data = [];
				var POST_href = window.location.pathname + '?link=comments&action=save';

				this.inputPhoto = this.element.querySelector('[type="file"]');
				this.photo = new Promise(function (resolve, reject) {
					var fileReader = new FileReader();
					if (_this4.inputPhoto.files[0]) {

						fileReader.readAsDataURL(_this4.inputPhoto.files[0]);
						fileReader.addEventListener('load', function (event) {
						...
						// ЕСЛИ КОММЕНТ ОБЯЗАТЕЛЕН, ТО УБРАТЬ ЭТОТ if
						if (true) {
							resolve('');
							return;
						}
						var rejected = reject;
						_this4.prom2 = _this4.showNO(_this4.element.querySelector('.productCard__commentConstructor-photo'), rejected);
						_this4.element.querySelector('.productCard__commentConstructor-photoPrewiev').classList.add('input--error');

						setTimeout(function () {
							return _this4.element.querySelector('.productCard__commentConstructor-photoPrewiev').classList.remove('input--error');
						}, 1000);
					}
				});

				this.name = new Promise(function (resolve, reject) {
					_this4.nameInput = _this4.element.querySelector('.productCard__commentConstructor-name input');
					if (_this4.nameInput.value) {
						// sendData += '&name=' + encodeURIComponent(this.nameInput.value);
						data.push({ name: 'name', value: _this4.nameInput.value });
						resolve();
					} else {
						// ЕСЛИ КОММЕНТ ОБЯЗАТЕЛЕН, ТО УБРАТЬ ЭТОТ if
						if (true) {
						...
						var rejected = reject;
						_this4.prom4 = _this4.showNO(_this4.element.querySelector('.productCard__commentConstructor-name'), rejected);
						_this4.nameInput.classList.add('input--error');

						setTimeout(function () {
							_this4.nameInput.classList.remove('input--error');
						}, 1000);
					}
				});

				this.mail = new Promise(function (resolve, reject) {
					_this4.mailInput = _this4.element.querySelector('.productCard__commentConstructor-mail input');
					if (_this4.mailInput.value) {
						// sendData += '&mail=' + encodeURIComponent(this.mailInput.value);
						data.push({ name: 'mail', value: _this4.mailInput.value });
						resolve();
					} else {
						// ЕСЛИ КОММЕНТ ОБЯЗАТЕЛЕН, ТО УБРАТЬ ЭТОТ if
						if (true) {
						...
						var rejected = reject;
						_this4.prom5 = _this4.showNO(_this4.element.querySelector('.productCard__commentConstructor-mail'), rejected);
						_this4.mailInput.classList.add('input--error');

						setTimeout(function () {
							_this4.mailInput.classList.remove('input--error');
						}, 1000);
					}
				});

				this.stars = new Promise(function (resolve, reject) {
					var stars = _this4.element.querySelector('[name="stars__daszcDD"]:checked').value;
					if (stars) {
						...
						var rejected = reject;
						_this4.prom6 = _this4.showNO(_this4.element.querySelector('.productCard__commentConstructor-starsContainer'), rejected);
					}
				});

				this.text = new Promise(function (resolve, reject) {
					_this4.textarea = _this4.element.querySelector('.productCard__commentConstructor-text textarea');
					if (_this4.textarea.value) {
						// sendData += '&text=' + encodeURIComponent(this.textarea.value);
						data.push({ name: 'text', value: _this4.textarea.value });
						resolve();
					} else {

						var rejected = reject;
						_this4.prom8 = _this4.showNO(_this4.element.querySelector('.productCard__commentConstructor-text'), rejected);
						_this4.textarea.parentNode.classList.add('input--error');

						setTimeout(function () {
							return _this4.textarea.parentNode.classList.remove('input--error');
						}, 1000);
					}
				});

				var a = [this.name, this.photo, this.text, this.stars, this.mail];
				...
				Promise.all(a).then(function (value) {
					...
					var button = _this4.element.querySelector('.productCard__commentConstructor-buttonSet');
					...
					//
					// 		setTimeout(()=> {
					// 			this.element.classList.add('productCard__commentConstructor--sended');
					// 			document.getElementById('hideCommentConstructor__fafTQw').checked = true;
					// 		},5000)
					// 	},500)
					// }
					// req.send(sendData);
					...
					$.ajax({
						...
						complete: function complete() {
							...
							this.mailInput.disabled = true;
							this.inputPhoto.disabled = true;
							var stars = this.element.querySelectorAll('[name="stars__daszcDD"]');
							for (var i = 0; i < stars.length; i++) {
								stars[i].disabled = true;
							}
							setTimeout(function () {
								...
								setTimeout(function () {
									_this5.element.classList.add('productCard__commentConstructor--sended');
									document.getElementById('hideCommentConstructor__fafTQw').checked = true;
								}, 5000);
							}, 500);
						}
					...
					_this4.setButtonClick = false;
				});
			}
		}, {
			key: 'closeButtonLogic',
			value: function closeButtonLogic() {
				// нету логики
			}
		}, {
			key: 'POSTProgress',
			value: function POSTProgress() {}
		}, {
			key: 'showNO',
			value: function showNO(parent, promStat) {
				// startCoordClass это css с изначальным положением картинки
				return this.setRed(parent).then(function () {
					return promStat(new Error('Не вышло'));
				});
			}
		}, {
			key: 'setRed',
			value: function setRed(parent) {
				var _this6 = this;
				...
				return new Promise(function (resolve, reject) {
					...
					}, _this6.timing + Math.random() * 2 * _this6.timingDelay);
				});
			}
		}]);

		return commentConstructorButtons;
	}();
...
/***/function (module, exports) {
	...
	module.exports = function () {
		...
		_createClass(pageDraw, [{
			key: 'isChildren',
			value: function isChildren(code) {
				for (var i = 0; i < this.childrenArray.length; i++) {
					if (this.childrenArray[i] == code) return true;else return false;
				}
			}
		}, {
			key: 'loadJson',
			...
			key: 'ajaxCacheControl',
			value: function ajaxCacheControl(url) {
				...
				if (!this[url]) {
					...
					console.log(this[url], url, "скачано");
				}
				return this[url];
			}
		}, {
			...
			set: function set(url) {
				var a = this.ajaxCacheControl(url) || Promise.resolve();
				if (this.__cache != a) {
			...
			set: function set(object) {
				var a = this.childrenArray.length || 1;
			...
			get: function get() {
				...
				return this.data.then(function () {
					...
					if (!_this8.drawed) {
						...
						if (_this8.chengeCache) {
							_this8.delete;
							_this8.chengeCache = false;
						}

						if (_this8.pos[1] instanceof Object) {

							_this8.pos[1].insertAdjacentElement(_this8.pos[0], _this8.content);
							_this8.drawed = true;
						} else {

							var element = document.querySelector(_this8.pos[1]);

							if (element) {
								element.insertAdjacentElement(_this8.pos[0], _this8.content);
								_this8.drawed = true;
							}
						}
						console.log('Отрисовано', _this8.content);

						if (_this8.show) _this8.show();
					} else {

						console.log('Не отрисовано', _this8.content);
					}

					if (!!_this8.children.length) {

						for (var i = 0; i < _this8.children.length; i++) {
							_this8.children[i].draw;
						}
					}
					if (_this8.callback) _this8.callback();
				}).catch(function (err) {
					throw err;
				});
			}
	...
	var productCard = function () {
		function productCard(options) {
			...
			for (var i = 0; i < this.photos.length; i++) {
				this.photos[i].addEventListener('click', function () {

					for (var m = 0; m < _this9.photos.length; m++) {
						_this9.photos[m].classList.remove('productCard__miniPhotos-listItem--active');
					}

					_this9.bigPhoto.src = event.currentTarget.querySelector('img').getAttribute('data-big-photo-src');
					var hrefForDownload = event.currentTarget.querySelector('img').getAttribute('data-big-photo-download-href') || alert('Оригинала нет');
					var a = document.querySelector('.productCard-download').setAttribute('href', hrefForDownload);
			...
			for (var i = 0; i < this.lists.length; i++) {
				...
				this.lists[i].addEventListener('click', function (event) {

					_this9.toggleDropList(event);
				});
			}
		...
		_createClass(productCard, [{
			key: 'toggleDropList',
			value: function toggleDropList(event) {
				var element = event.currentTarget.querySelector('[data-style="dropList"]');
				if (element.style.display == 'none' || getComputedStyle(element).display == "none") {

					element.style.display = 'block';
					var clickOpen = new Click({
						element: element.querySelector('ul'),
						callback: function callback(event) {

							var content = event.target.closest('li');
							...
							var result = event.currentTarget.closest('span').querySelector('[data-droplist-result]');
							result.innerHTML = content.innerHTML;
							var attributs = content.dataset;
			...
			key: 'setOrangeColorForAction',
			value: function setOrangeColorForAction() {
...
/***/function (module, exports, __webpack_require__) {
	...
	var pigPrintsCarousel = function () {
		function pigPrintsCarousel() {
			_classCallCheck(this, pigPrintsCarousel);

			this.arrow = '<svg viewBox="0 0 48 48"><use xlink:href="#icon-arrow"></use></svg>';
			this.initBigPrintCarousel();
		}

		_createClass(pigPrintsCarousel, [{
			key: 'initBigPrintCarousel',
			value: function initBigPrintCarousel() {
...
/***/function (module, exports) {
	var advantages = function () {
		function advantages() {
			...
			var closes = document.querySelectorAll('.advantages__infoBlocks-item--close');
