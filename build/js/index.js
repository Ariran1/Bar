...
/***/ (function(module, exports, __webpack_require__) {
	...
	__webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"./productCard/js/productCard.js\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
	// require('./elements/dropList/dropList.js');
	__webpack_require__(4);
	__webpack_require__(6);

	var owlCarousel = __webpack_require__(5);
...
/***/ (function(module, exports, __webpack_require__) {
		...
		setImage() {
			...
			this.input.addEventListener("change", () => {
				...
				if (this.input.files[0].type !== 'image/png'
				 && this.input.files[0].type !== 'image/jpeg') {
				 	this.input.value = '';
					return;
				}
		        ...
		        this.photoText.innerHTML = 'Удалить фото';
			...
			this.input.addEventListener('click',(event) => {
		    	...
		    	this.photoText.innerHTML = 'Загрузить фото';
	...
	let pageDraw = __webpack_require__(2);
	...
	class loadComments {
		...
		chengeCommentsPage(event) {
			...
			.then((data)=>{
	            ...
	            <p style=" padding: 20px;">Здесь пока нет отзыва, <span style="color:#0054a6;cursor:pointer;" onclick="document.querySelector('#showCommentConstructor__fafTQw').checked = true;">оставьте</span> свой!</p>
	          </div>`;
					return null;
				}
		...
		setButtonLogic() {
			...
			this.mail = new Promise((resolve,reject) => {
				...
				if (this.mailInput.value) {
					...
					var rejected = reject;
					this.prom5 = this.showNO(this.element.querySelector('.productCard__commentConstructor-mail'),rejected);
					this.mailInput.classList.add('input--error');

					setTimeout(() => {
						this.mailInput.classList.remove('input--error');
					},1000);
				}
			});
			...
			this.text = new Promise((resolve,reject) => {
				...
				if (this.textarea.value) {
					...
					this.textarea.parentNode.classList.add('input--error');

					setTimeout(() => this.textarea.parentNode.classList.remove('input--error'),
						1000);
				...
				$.ajax({
												...
												complete: function () {
													self.textarea.disabled = true;
														...
														self.mailInput.disabled = true;
														self.inputPhoto.disabled = true;
														let stars = self.element.querySelectorAll('[name="stars__daszcDD"]');
														for (var i = 0; i < stars.length; i++) {
															stars[i].disabled = true;
														}
														setTimeout(()=>{
															...
															setTimeout(()=> {
																self.element.classList.add('productCard__commentConstructor--sended');
																document.getElementById('hideCommentConstructor__fafTQw').checked = true;
															},5000);
														},500);
												}
